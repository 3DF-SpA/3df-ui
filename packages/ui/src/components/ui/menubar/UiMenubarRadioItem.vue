...
import {
  MENUBAR_ITEM_CONTEXT_KEY,
  MENUBAR_MENU_KEY,
  MENUBAR_RADIO_GROUP_KEY,
} from './menubar-types';

defineOptions({ name: 'UiMenubarRadioItem', inheritAttrs: false });

interface UiMenubarRadioItemProps {
  value: string;
  disabled?: boolean;
}

const props = withDefaults(defineProps<UiMenubarRadioItemProps>(), {
  disabled: false,
});
...
const radioGroup = inject(MENUBAR_RADIO_GROUP_KEY)!;
